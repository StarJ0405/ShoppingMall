...
export default function Main({children,className,user}: Readonly<{children : React.ReactNode,className?:string,user?:any}>) {
    ...
    <main id='main' className={'min-h-screen flex flex-col items-center realtive '+className}>
      <header className="flex w-[1240px] items-center h-[120px]">
        ...
        <div className="justify-self-end flex ml-[150px] justify-between w-[300px]" >
          <a href=""><img id="user" alt="user" src="/user.png" className="w-[48px] h-[48px]" onMouseEnter={e=>{(e.target as any).src='/user_red.png'; openUserHover();}} onMouseLeave={e=>{(e.target as any).src='/user.png'; closeUserHover();}}></img></a>
      ...
      <DropDown open={userHover} onClose={()=>closeUserHover()} className="" background="main" button="user" defaultDriection={Direcion.DOWN} height={205} width={170}>
        <div className="h-full w-full flex flex-col justify-between py-4 px-2" onMouseEnter={()=>openUserHover()} onMouseLeave={()=>closeUserHover()}>
          <a href="" className="text-lg" onMouseEnter={e=>(e.target as HTMLElement).classList.add("text-red-500")} onMouseLeave={e=>(e.target as HTMLElement).classList.remove("text-red-500")}>주문/배송조회</a>
          <a href="" className="text-lg" onMouseEnter={e=>(e.target as HTMLElement).classList.add("text-red-500")} onMouseLeave={e=>(e.target as HTMLElement).classList.remove("text-red-500")}>취소/반품/교환</a>
          <a href="" className="text-lg" onMouseEnter={e=>(e.target as HTMLElement).classList.add("text-red-500")} onMouseLeave={e=>(e.target as HTMLElement).classList.remove("text-red-500")}>고객센터</a>
          <a href="" className="text-lg" onMouseEnter={e=>(e.target as HTMLElement).classList.add("text-red-500")} onMouseLeave={e=>(e.target as HTMLElement).classList.remove("text-red-500")}>회원정보</a>
        </div>
      </DropDown>
      ...
      <nav className="flex w-[1240px] h-[66px] items-center justify-between">
        <div className="flex items-center">
          <a href="" className="text-lg border-red-500" onMouseEnter={e=>(e.target as HTMLElement).classList.add('border-b-2')} onMouseLeave={e=>(e.target as HTMLElement).classList.remove('border-b-2')}>베스트</a>
        </div>
        {user?
          <a href="" className='font-bold' onMouseEnter={e=>(e.target as HTMLElement).classList.add('underline')} onMouseLeave={e=>(e.target as HTMLElement).classList.remove('underline')}>user?.nickname</a>
        :
          <a href="/account/login" className="font-bold" onMouseEnter={e=>(e.target as HTMLElement).classList.add('underline')} onMouseLeave={e=>(e.target as HTMLElement).classList.remove('underline')}>로그인</a>
        }
