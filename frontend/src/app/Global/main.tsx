    ...
    <main id='main' className={'min-h-screen flex flex-col items-center realtive '+className}>
      ...
      <DropDown open={userHover} onClose={()=>closeUserHover()} className="" background="main" button="user" defaultDriection={Direcion.DOWN} height={205} width={170}>
        <div className="h-full w-full flex flex-col justify-between py-4 px-2" onMouseEnter={()=>openUserHover()} onMouseLeave={()=>closeUserHover()}>
          <a href="" className="text-lg hover:text-red-500">주문/배송조회</a>
          <a href="" className="text-lg hover:text-red-500">취소/반품/교환</a>
          <a href="" className="text-lg hover:text-red-500">고객센터</a>
          <a href="/account/profile" className="text-lg hover:text-red-500">회원정보</a>
          {user? <a href="/" onClick={e=>{e.preventDefault(); localStorage.clear(); window.location.reload();}} className="text-lg" onMouseEnter={e=>(e.target as HTMLElement).classList.add("text-red-500")} onMouseLeave={e=>(e.target as HTMLElement).classList.remove("text-red-500")}>로그아웃</a>:<></>}
        </div>
      </DropDown>
