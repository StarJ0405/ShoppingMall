...
export default function Page(props : pageProps){
    ...
    return <Profile user={user}>
                ...
                {widhList?.map((wish,index)=>
                    <tr className="h-[64px] align-middle" key={index}>
                        ...
                        <td className="text-xs">
                            <button className="px-2 border border-black mb-1">장바구니</button>
                            <button className="px-2 border border-black" onClick={()=>Delete(wish.id)}>삭제하기</button>
                        </td>
                    </tr>
                )}
            </tbody>
        ...
        <div className="text-xs flex">
            <button className="border border-black mb-1 w-[132px] h-[18px]">선택상품 장바구니 담기</button>
            <button className="border border-black w-[66px] h-[18px]" onClick={()=>DeleteAll()}>삭제하기</button>            
        </div>
    </Profile>
}