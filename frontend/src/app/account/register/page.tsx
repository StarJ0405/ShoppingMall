...
export default function Page(){
    ...
    function IsDisabled(){
        return username=='' || password=='' || realname=='' || email == ''||nickanme=='' || phone=='' ||role==-1 || birthday==''|| gender==-1;
    }
    ...
    return <div className='w-full h-full flex justify-center'>
        <div className='flex flex-col w-[960px] pt-[20px] items-center'>
            ...
            <div className='w-[400px] flex flex-col justify-center'>
                ...
                <input id='nickname' type='text' className='w-[400px] text-xl mt-[24px]' placeholder='닉네임' onFocus={e=>{e.target.placeholder='';setFocusing(4)}} onBlur={e=>e.target.placeholder='닉네임'} onKeyDown={e=>{KeyDownCheck({preKey,setPreKey, e:e,pre:()=>Move('email'),next:()=>Move('phone')})}} onChange={e=>setNickanme(e.target.value)} maxLength={24} onKeyUp={e=>check(e,"^[A-Za-z가-힣0-9_\s]{2,24}$","닉네임은 한글,영어,숫자만 가능하며 최대 24자까지 가능합니다.")}/>
                <input id='phone' type='text' className='w-[400px] text-xl mt-[24px]' placeholder='전화번호' onFocus={e=>{e.target.placeholder='';setFocusing(5)}} onBlur={e=>e.target.placeholder='전화번호'} onKeyDown={e=>{KeyDownCheck({preKey,setPreKey, e:e,pre:()=>Move('nickname'),next:()=>Move('role')})}} onChange={e=>setPhone(e.target.value)} onKeyUp={e=>check(e,"^0[0-9]{2}-[0-9]{4}-[0-9]{4}$","전화번호 형식이 맞지 않습니다.(###-####-####)")}/>
                <select id='role' defaultValue={-1} className='w-[400px] text-xl mt-[24px]' onFocus={()=>setFocusing(6)} onKeyDown={e=>{KeyDownCheck({preKey,setPreKey, e:e,pre:()=>Move('phone'),next:()=>Move('birthday')})}} onChange={e=>setRole(e.target.selectedIndex)}>
