...
import { KeyDownCheck, Move, PhoneNumberCheck, checkInput } from '@/app/Global/Method';
...
export default function Page() {
    ...
    function Submit() {
        SignUp({ username: username, password: password, name: name, email: email, nickname: nickname, phoneNumber: phoneNumber, role: role, birthday: birthday, gender: gender })
            .then(() => {
    ...
    return <div className='w-full h-full flex justify-center'>
        <div className='flex flex-col w-[960px] pt-[20px] items-center'>
            ...
            <div className='w-[400px] flex flex-col justify-center'>
                ...
                <input id='password' type='password' className='w-[400px] text-xl mt-[24px]' placeholder='비밀번호' onFocus={e => { e.target.placeholder = ''; setFocusing(1) }} onBlur={e => e.target.placeholder = '비밀번호'} onKeyDown={e => { KeyDownCheck({ preKey, setPreKey, e: e, pre: () => Move('username'), next: () => Move('name') }) }} onChange={e => setPassword(e.target.value)} maxLength={24} onKeyUp={e => checkInput(e, '^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[!@#$%^&*()-+={}~?:;`|/]).{6,24}$', () => setError(''), () => setError('비밀번호는 최소 6, 최대 24자로 대문자, 소문자, 숫자, 특수문자(!@#$%^&*()-+={}~?:;`)가 각각 한개씩 들어가 있어야합니다.'))} />
                <input id='name' type='text' className='w-[400px] text-xl mt-[24px]' autoFocus placeholder='이름' onFocus={e => { e.target.placeholder = ''; setFocusing(2) }} onBlur={e => e.target.placeholder = '이름'} onKeyDown={e => { KeyDownCheck({ preKey, setPreKey, e: e, pre:()=>Move('password'), next: () => Move('birthday') }); }} onChange={e => setName(e.target.value)} maxLength={5} onKeyUp={e => checkInput(e, '^([가-힣]){2,}$', () => setError(''), () => setError( '이름은 한글 2글자 이상으로 구성되어야 합니다.'))} />
